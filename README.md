...
- `rm <path>`：默认仅移除当前平台登记。可传入其安装路径（符号链接）或 `~/dotfiles` 中的保存路径；工具会移除链接并恢复文件。若该保存项仍被其他系统使用，会复制内容到当前安装路径。加 `--all` 会删除所有平台登记和保存对象：当前平台已登记时先恢复到其本地安装路径，未登记时直接删除保存对象；绝不访问其他机器的安装路径。
